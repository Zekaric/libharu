...
static const HpdfPoint     INIT_POS  = { 0, 0 };
   ...
   HPDF_LineJoin  line_join)
{
   ...
   if (line_join >= HPDF_LINEJOIN_EOF)
      return HPDF_RaiseError(page->error, HPDF_PAGE_OUT_OF_RANGE,
         (HpdfStatus) line_join);

   attr = (HPDF_PageAttr) page->attr;

   if (HPDF_Stream_WriteInt(attr->stream, (HpdfUInt) line_join) != HPDF_OK)
      return HPDF_CheckError(page->error);
   ...
   HpdfReal        phase)
{
   ...
   const HpdfReal *pdash_ptn = dash_ptn;
   HPDF_PageAttr attr;
   HpdfUInt i;
...
HPDF_Page_LineTo(HPDF_Page  page,
   ...
   HpdfReal  y)
{
   HpdfStatus ret = HPDF_Page_CheckState(page, HPDF_GMODE_PATH_OBJECT);
   char buf[HPDF_TMP_BUF_SIZ];
   char *pbuf = buf;
   char *eptr = buf + HPDF_TMP_BUF_SIZ - 1;
   HPDF_PageAttr attr;
   ...
   HpdfReal  size)
{
   ...
   if (size <= 0 || size > HPDF_MAX_FONTSIZE)
      return HPDF_RaiseError(page->error, HPDF_PAGE_INVALID_FONT_SIZE,
         (HpdfStatus) size);
   ...
   HPDF_TextRenderingMode  mode)
{
   ...
   if (HPDF_Stream_WriteInt(attr->stream, (HpdfInt) mode) != HPDF_OK)
      return HPDF_CheckError(page->error);
...
static const HpdfReal KAPPA = 0.552F;
   ...
   HpdfReal    ang2)
{
   ...
   for (;;) {
      if (fabs(ang2 - ang1) <= 90)
         ...
         HpdfReal tmp_ang = (ang2 > ang1 ? ang1 + 90 : ang1 - 90);
   ...
   HpdfBool    cont_flg)
{
   ...
   HpdfDouble rx0, ry0, rx1, ry1, rx2, ry2, rx3, ry3;
   HpdfDouble x0, y0, x1, y1, x2, y2, x3, y3;
   HpdfDouble delta_angle;
   HpdfDouble new_angle;
   ...
   if (!cont_flg) {
      pbuf = HPDF_FToA(pbuf, (HpdfReal) x0, eptr);
      *pbuf++ = ' ';
      pbuf = HPDF_FToA(pbuf, (HpdfReal) y0, eptr);
   ...
   pbuf = HPDF_FToA(pbuf, (HpdfReal) x1, eptr);
   *pbuf++ = ' ';
   pbuf = HPDF_FToA(pbuf, (HpdfReal) y1, eptr);
   *pbuf++ = ' ';
   pbuf = HPDF_FToA(pbuf, (HpdfReal) x2, eptr);
   *pbuf++ = ' ';
   pbuf = HPDF_FToA(pbuf, (HpdfReal) y2, eptr);
   *pbuf++ = ' ';
   pbuf = HPDF_FToA(pbuf, (HpdfReal) x3, eptr);
   *pbuf++ = ' ';
   pbuf = HPDF_FToA(pbuf, (HpdfReal) y3, eptr);
   HPDF_StrCpy(pbuf, " c\012", eptr);

   if ((ret = HPDF_Stream_WriteStr(attr->stream, buf)) != HPDF_OK)
      return HPDF_CheckError(page->error);

   attr->cur_pos.x = (HpdfReal) x3;
   attr->cur_pos.y = (HpdfReal) y3;
   attr->str_pos = attr->cur_pos;
   attr->gmode = HPDF_GMODE_PATH_OBJECT;
      ...
      char const       *text)
{
   ...
   if (font_attr->type == HPDF_FONT_TYPE0_TT ||
      ...
      if (encoder->encode_text_fn == NULL) {
         if ((ret = HPDF_Stream_WriteBinary(attr->stream, (HpdfByte *) text,
            len, NULL))
            != HPDF_OK)
            return ret;
      }
         ...
         ret = HPDF_Stream_WriteBinary(attr->stream, (HpdfByte *) encoded,
            length, NULL);
   ...
   HpdfReal *yRel)
{
   if (text_matrix.a == 0) {
      ...
      *yRel  = (xAbs - text_matrix.x) / text_matrix.c;
      ...
      HpdfReal y = (yAbs - text_matrix.y - (xAbs - text_matrix.x) *
...
)
{
   ...
   HpdfUInt num_rest;
   HPDF_Box bbox;
   HpdfBool char_space_changed = HPDF_FALSE;
