...
static HpdfByte const HPDF_PADDING_STRING[] = {
...
static void
MD5Transform(HpdfUInt32       buf[4],
   const HpdfUInt32 in[16]);
...
{
   HpdfUInt32 t;
   HpdfByte const *bufTemp = buf;

   /* Update bitcount */
   t = ctx->bits[0];
   if ((ctx->bits[0] = t + ((HpdfUInt32) len << 3)) < t)
   ...
   if (t) {
      HpdfByte *p = (HpdfByte *) ctx->in + t;

      t = 64 - t;
      if (len < t)
      {
         HPDF_MemCpy(p, bufTemp, len);
         return;
      }
      HPDF_MemCpy(p, bufTemp, t);
   ...
   struct HPDF_MD5Context *ctx)
{
   ...
   *p++ = 0x80;
   ...
   ((HpdfUInt32 *) ctx->in)[14] = ctx->bits[0];
   ((HpdfUInt32 *) ctx->in)[15] = ctx->bits[1];
      ...
      HpdfUInt32  longs)
{
   ...
   do
   {
      t = (HpdfUInt32) ((HpdfUInt32) bufTemp[3] << 8 | bufTemp[2]) << 16 |
         ((HpdfUInt32) bufTemp[1] << 8 | bufTemp[0]);
      *(HpdfUInt32 *) bufTemp = t;
      bufTemp += 4;
   } while (--longs);
}
   ...
   HpdfByte        *const new_pwd)
{
   ...
   if (len >= HPDF_PASSWD_LEN) {
      ...
      if (len > 0) {
         HPDF_MemCpy(new_pwd, (HpdfByte *) pwd, len);
      }
...
HPDF_Encrypt_CreateOwnerKey(HPDF_Encrypt  attr)
{
   ...
   HpdfByte digest[HPDF_MD5_KEY_LEN];
   HpdfByte tmppwd[HPDF_PASSWD_LEN];
   ...
   if (attr->mode == HPDF_ENCRYPT_R3) {
      HpdfByte tmppwd2[HPDF_PASSWD_LEN];
      HpdfUInt i;

      for (i = 1; i <= 19; i++) {
         HpdfUInt j;
         HpdfByte new_key[HPDF_MD5_KEY_LEN];

         for (j = 0; j < attr->key_len; j++)
            new_key[j] = (HpdfByte) (digest[j] ^ i);
...
HPDF_Encrypt_CreateEncryptionKey(HPDF_Encrypt  attr)
{
   ...
   tmp_flg[0] = (HpdfByte) (attr->permission);
   tmp_flg[1] = (HpdfByte) (attr->permission >> 8);
   tmp_flg[2] = (HpdfByte) (attr->permission >> 16);
   tmp_flg[3] = (HpdfByte) (attr->permission >> 24);
...
HPDF_Encrypt_CreateUserKey(HPDF_Encrypt  attr)
{
   ...
   if (attr->mode == HPDF_ENCRYPT_R3) {
      HPDF_MD5_CTX md5_ctx;
      HpdfByte digest[HPDF_MD5_KEY_LEN];
      HpdfByte digest2[HPDF_MD5_KEY_LEN];
      HpdfUInt i;
      ...
      for (i = 1; i <= 19; i++) {
         ...
         for (j = 0; j < attr->key_len; j++)
            new_key[j] = (HpdfByte) (attr->encryption_key[j] ^ i);
   ...
   HpdfUInt          len)
{
   ...
   for (i = 0; i < len; i++) {
      ...
      ctx->idx1 = (HpdfByte) ((ctx->idx1 + 1) % 256);
      ctx->idx2 = (HpdfByte) ((ctx->idx2 +  ctx->state[ctx->idx1]) % 256);
   ...
   HpdfUInt16       gen_no)
{
   ...
   attr->encryption_key[attr->key_len]     = (HpdfByte) object_id;
   attr->encryption_key[attr->key_len + 1] = (HpdfByte) (object_id >> 8);
   attr->encryption_key[attr->key_len + 2] = (HpdfByte) (object_id >> 16);
   attr->encryption_key[attr->key_len + 3] = (HpdfByte) gen_no;
   attr->encryption_key[attr->key_len + 4] = (HpdfByte) (gen_no >> 8);

   HPDF_PTRACE(("@@@ OID=%u, gen_no=%u\n", (HpdfInt) object_id, gen_no));
...
{
   HpdfUInt key_len = (attr->key_len + 5 > HPDF_ENCRYPT_KEY_MAX) ?
      HPDF_ENCRYPT_KEY_MAX : attr->key_len + 5;
