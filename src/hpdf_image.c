      ...
      HPDF_Xref        xref)
{
   ...
   for (;;) {
      ...
      ret = HPDF_Stream_Read(jpeg_data, buf, &len);
      ...
      HpdfUInt          bits_per_component)
{
   ...
   case HPDF_CS_DEVICE_GRAY:
      size = (HpdfUInt) ((HpdfDouble) width * height / (8 / bits_per_component) + 0.876);
      ret = HPDF_Dict_AddName(image, "ColorSpace", COL_GRAY);
      break;
      ...
      HPDF_Image  image)
{
   ...
   if (!subtype || 
       !HpdfStrIsEqual(subtype->value, "Image"))
   {
      HPDF_RaiseError(image->error, HPDF_INVALID_IMAGE, 0);
      return HPDF_FALSE;
   }
      ...
      HPDF_Image image)
{
   ...
   if (width && height)
   {
      ret.x = (HpdfReal) width->value;
      ret.y = (HpdfReal) height->value;
   }
      ...
      HpdfPoint * const size)
{
   ...
   if (width &&
      height)
   {
      size->x = (HpdfReal) width->value;
      size->y = (HpdfReal) height->value;
   }
...
{
   return (HpdfUInt) HPDF_Image_GetSize(image).x;
}

HPDF_EXPORT(HpdfUInt)
HPDF_Image_GetHeight(HPDF_Image   image)
{
   return (HpdfUInt) HPDF_Image_GetSize(image).y;
}
