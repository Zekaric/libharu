   ...
   HPDF_Xref         xref)
{
   HpdfStatus   ret;
   HPDF_Encrypt attr = (HPDF_Encrypt) dict->attr;
   HpdfObjBinary  *user_key;
   HpdfObjBinary  *owner_key;
   ...
   owner_key = HpdfObjBinaryCreate(dict->mmgr, attr->owner_key, HPDF_PASSWD_LEN);
   if (!owner_key)
   {
   ...
   user_key = HpdfObjBinaryCreate(dict->mmgr, attr->user_key, HPDF_PASSWD_LEN);
   if (!user_key)
   {
      return HPDF_Error_GetCode(dict->error);
   }
...
{
   HpdfObjHeader *header = (HpdfObjHeader *) dict;
