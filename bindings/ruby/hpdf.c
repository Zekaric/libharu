...
static void hpdf_free(HpdfDoc const * const pdf);
...
static VALUE
   hpdf_new()
{
   HpdfDoc *pdf = HPDF_New(hpdf_error_handler, NULL);

   HPDF_PTRACE(("hpdf_new pdf=%p\n", pdf));

   if (!pdf)
   {
      rb_raise(rb_eHPDFError, "cannot create HpdfDoc const * const object.");
   }
...
static VALUE
   hpdf_set_page_configuration(
      VALUE obj,
      VALUE page_per_pages)
{
   HpdfDoc *pdf;
   HpdfStatus ret;
   HpdfUInt i;
      ...
      VALUE page)
{
   ...
   if (!HpdfStrIsEqual(rb_obj_classname(page), "HPDFPage"))
   {
      rb_raise(rb_eHPDFError, "invalid argument(1).");
   }
...
static VALUE
   hpdf_page_create_uri_link_annot(
      VALUE obj, 
      VALUE left, 
      VALUE bottom, 
      VALUE right, 
...
static VALUE
   hpdf_load_jpeg_image_from_file(
      VALUE obj, 
      VALUE file_name)
{
   HpdfDoc *pdf;
...
hpdf_image_set_mask_image(VALUE obj, VALUE mask_image)
{
   ...
   if (!HpdfStrIsEqual(rb_obj_classname(mask_image), "HPDFImage"))
   {
      rb_raise(rb_eHPDFError, "invalid argument(1).");
   }
...
static VALUE
   hpdf_page_get_current_text_pos(
      VALUE obj)
{
   HPDF_Page page;
   HpdfPoint p;
...
hpdf_page_set_font_and_size(VALUE obj, VALUE font, VALUE size)
{
   ...
   if (!HpdfStrIsEqual(rb_obj_classname(font), "HPDFFont"))
   {
      rb_raise(rb_eHPDFError, "invalid argument(1).");
   }
...
hpdf_page_execute_xobject(VALUE obj, VALUE xobj)
{
   ...
   if (!HpdfStrIsEqual(rb_obj_classname(xobj), "HPDFImage"))
   {
      rb_raise(rb_eHPDFError, "invalid argument(1).");
   }
...
hpdf_page_draw_image(VALUE obj, VALUE image, VALUE x, VALUE y, VALUE width, VALUE height)
{
   ...
   if (!HpdfStrIsEqual(rb_obj_classname(image), "HPDFImage"))
   {
      rb_raise(rb_eHPDFError, "invalid argument(1).");
   }
