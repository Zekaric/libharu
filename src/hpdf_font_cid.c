...
static HPDF_Dict
CreateCMap(HpdfEncoder const * const   encoder,
   HPDF_Xref      xref);
      ...
      HPDF_Xref        xref)
{
   ...
   */
      if (HpdfStrIsEqual(encoder_attr->ordering, "Identity-H"))
      {
      ...
      HPDF_Xref xref)
{
   ...
   HpdfEncoder const * const   encoder       = attr->encoder;
   ...
   {
      HPDF_CID_Width *w = (HPDF_CID_Width *) HPDF_List_ItemAt(fontdef_attr->widths, i);
      ...
      HPDF_Xref xref)
{
   ...
   for (i = 0; i < 256; i++) 
   {
      ...
      for (j = 0; j < 256; j++) 
      {
         ...
         {
            HpdfUInt16 cid = encoder_attr->cid_map[i][j];
            if (cid != 0) 
            {
               HpdfUnicode unicode = encoder_attr->unicode_map[i][j];
         ...
         {
            HpdfUnicode unicode = (HpdfUnicode) ((i << 8) | j);
            HpdfUInt16  gid     = HPDF_TTFontDef_GetGlyphid(fontdef, unicode);
   ...
   if (max > 0) 
   {
      ...
      {
         HpdfInt w = HPDF_TTFontDef_GetGidWidth(fontdef, *ptmp_map);
      ...
      if (fontdef_attr->embedding) 
      {
         ...
         {
            HpdfByte   u[2];
            HpdfUInt16 gid = tmp_map[i];

            u[0] = (HpdfByte) (gid >> 8);
            u[1] = (HpdfByte) gid;

            HPDF_MemCpy((HpdfByte *) (tmp_map + i), u, 2);
         }

         ret = HPDF_Stream_Write(attr->map_stream->stream, (HpdfByte *) tmp_map, max * 2);
         if (ret != HPDF_OK)
         {
            return NULL;
         }
   ...
   HpdfUInt         len)
{
   ...
   if (attr->fontdef->type == HPDF_FONTDEF_TYPE_CID) {
      ...
      dw2 = (HpdfInt) (attr->fontdef->font_bbox.bottom -
         attr->fontdef->font_bbox.top);
   }

   HPDF_Encoder_SetParseText(encoder, &parse_state, textTemp, len);
   ...
   HpdfReal         *real_width)
{
   ...
   {
      HpdfByte b  = *textTemp++;
      HpdfByte b2 = *textTemp;  /* next byte */
      ...
      if (btype != HPDF_BYTE_TYPE_TRAIL) {
         if (attr->writing_mode == HPDF_WMODE_HORIZONTAL) {
            if (attr->fontdef->type == HPDF_FONTDEF_TYPE_CID) {
               /* cid-based font */
               HpdfUInt16 cid = HPDF_CMapEncoder_ToCID(encoder, code);
      ...
      w += (HpdfReal) ((HpdfDouble) tmp_w * font_size / 1000);
   ...
   HpdfByte    width)
{
   ...
   /* align byte-order */
   HPDF_MemCpy(b, (HpdfByte *) &val, 2);
   val2 = (HpdfUInt16) ((HpdfUInt16) b[0] << 8 | (HpdfUInt16) b[1]);

   HPDF_MemCpy(b, (HpdfByte *) &val2, 2);
      ...
      HPDF_Xref                   xref)
{
   ...
   *pbuf++ = ' ';
   ...
   pbuf =          HPDF_IToA(  pbuf, (HpdfUInt32) attr->writing_mode, eptr);
                   HPDF_StrCpy(pbuf, " def\r\n\r\n",                  eptr);
   ret += HPDF_Stream_WriteStr(cmap->stream, buf);
   ...
   {
      HPDF_CidRange_Rec *range = HPDF_List_ItemAt(attr->code_space_range, i);
   ...
   pbuf = (char *) HPDF_StrCpy(pbuf, "CMapName currentdict /CMap defineresource pop\r\n", eptr);
