...
static const HPDF_PageSizeValue HPDF_PREDEFINED_PAGE_SIZES[] = {
    ...
    {(HpdfReal) 841.89, (HpdfReal) 1190.551},    /* HPDF_PAGE_SIZE_A3 */
    {(HpdfReal) 595.276, (HpdfReal) 841.89},     /* HPDF_PAGE_SIZE_A4 */
    {(HpdfReal) 419.528, (HpdfReal) 595.276},     /* HPDF_PAGE_SIZE_A5 */
    {(HpdfReal) 708.661, (HpdfReal) 1000.63},     /* HPDF_PAGE_SIZE_B4 */
    {(HpdfReal) 498.898, (HpdfReal) 708.661},     /* HPDF_PAGE_SIZE_B5 */
...
static char const * const HPDF_INHERITABLE_ENTRIES[5] = {
                        "Resources",
                        "MediaBox",
                        "CropBox",
                        "Rotate",
   ...
   HPDF_Dict   kid)
{
   ...
   kids = (HpdfArray *) HPDF_Dict_GetItem(parent, "Kids", HPDF_OCLASS_ARRAY);
   if (!kids)
   {
...
GetPageCount(HPDF_Dict    pages)
{
   HpdfUInt i;
   HpdfUInt count = 0;
   HpdfArray *kids = (HpdfArray *) HPDF_Dict_GetItem(pages, "Kids", HPDF_OCLASS_ARRAY);
      ...
      HPDF_Xref   xref)
{
   ...
   attr = HpdfMemCreateType(page->mmgr, HPDF_PageAttr_Rec);
   if (!attr) 
   {
      HPDF_Dict_Free(page);
      return NULL;
   ...
   page->attr = attr;
   HpdfMemClearType(attr, HPDF_PageAttr_Rec);
   ...
   HpdfUInt16        obj_class)
{
   ...
   {
      if (HpdfStrIsEqual(key, HPDF_INHERITABLE_ENTRIES[i]))
      {
         chk = HPDF_TRUE;
         break;
      }
   ...
   HpdfBool      zoom)
{
   ...
   fromxobject = HPDF_DictStream_New(doc->mmgr, doc->xref);
   if (!fromxobject)
   {
      return NULL;
   }
   ...
   ** compatibility */
   ...
   if (zoom)
   {
      if (HPDF_Stream_WriteReal(fromxobject->stream, rectTemp.right - rectTemp.left) != HPDF_OK)
      {
         return NULL;
      }
      ...
      if (HPDF_Stream_WriteReal(fromxobject->stream, rectTemp.top - rectTemp.bottom) != HPDF_OK)
      {
         return NULL;
      }
   ...
   char const     *text)
{
   ...
   HPDF_TextWidth tw;
   HpdfReal ret = 0;
   HpdfUInt len = HPDF_StrLen(text, HPDF_LIMIT_MAX_STRING_LEN + 1);
   ...
   tw = HPDF_Font_TextWidth(attr->gstate->font, (HpdfByte *) text, len);
   ...
   HpdfReal         *real_width)
{
   ...
   ret = HPDF_Font_MeasureText(attr->gstate->font, (HpdfByte *) text, len, width,
      ...
      HpdfUInt16    angle)
{
   ...
   if (angle % 90 != 0)
   {
      return HPDF_RaiseError(page->error, HPDF_PAGE_INVALID_ROTATE_VALUE, (HpdfStatus) angle);
   }
   ...
   HPDF_PageDirection    direction)
{
   ...
   if (size < 0 || size > HPDF_PAGE_SIZE_EOF)
      return HPDF_RaiseError(page->error, HPDF_PAGE_INVALID_SIZE,
         (HpdfStatus) direction);

   if (direction == HPDF_PAGE_LANDSCAPE) {
      ret += HPDF_Page_SetHeight(page,
         HPDF_PREDEFINED_PAGE_SIZES[(HpdfUInt) size].x);
      ret += HPDF_Page_SetWidth(page,
         HPDF_PREDEFINED_PAGE_SIZES[(HpdfUInt) size].y);
   }
   ...
   HpdfRect const rect)
{
   ...
   fxobj = HPDF_Page_CreateXObjectAsWhiteRect(doc, page, rect);
   if (!fxobj)
   {
      return NULL;
   }
...
HPDF_Page_CreatePD33DMeasure(
   HPDF_Page       page,
   HpdfPoint3D const annotationPlaneNormal,
   HpdfPoint3D const firstAnchorPoint,
   HpdfPoint3D const secondAnchorPoint,
   HpdfPoint3D const leaderLinesDirection,
   HpdfPoint3D const measurementValuePoint,
   HpdfPoint3D const textYDirection,
   HpdfReal       value,
