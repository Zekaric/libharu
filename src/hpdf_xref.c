      ...
      HpdfUInt32   offset)
{
   ...
   xref = HpdfMemCreateType(mmgr, HPDF_Xref_Rec);
   if (!xref)
   {
      return NULL;
   }

   HpdfMemClearType(xref, HPDF_Xref_Rec);
   xref->mmgr = mmgr;
   ...
   if (xref->start_offset == 0) {
      new_entry = HpdfMemCreateType(mmgr, HPDF_XrefEntry_Rec);
      if (!new_entry)
      {
         goto Fail;
      }
   ...
   while (xref) {
      ...
      if (xref->entries) {
         for (i = 0; i < xref->entries->count; i++) {
            ...
            HpdfMemDestroy(xref->mmgr, entry);
         }
   ...
   void       *obj)
{
   ...
   entry = HpdfMemCreateType(xref->mmgr, HPDF_XrefEntry_Rec);
   if (entry == NULL)
   {
      goto Fail;
   }
